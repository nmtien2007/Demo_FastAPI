...
def get_user_infos(db: Session):
    return db.using_bind("slave").query(models.UserInfoTab).all()

def get_user_info_by_user_name(user_name, db: Session):
    model = db.using_bind("slave").query(models.UserInfoTab).filter(
...
def filter_user_infos(db: Session, user_name=None, phone=None, email=None):
    # user_ids = []
    ls_models = db.using_bind("slave").query(models.UserInfoTab)
...
def get_user_infos_by_ids(ids, db: Session):
    return list(db.using_bind("slave").query(models.UserInfoTab).filter(models.UserInfoTab.id.in_(ids)))
